...
interface ProductsSearchContentProps {
  locale: Locale;
}

function ProductsSearchContent({ locale }: ProductsSearchContentProps) {
...
export function ProductsContent({ locale }: ProductsContentProps) {
  return (
    <Suspense fallback={
      <main className="flex-1 flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-emerald-600"></div>
      ...
      <ProductsSearchContent locale={locale} />
    </Suspense>
  );
}